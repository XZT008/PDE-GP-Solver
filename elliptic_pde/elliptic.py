...
class GP_Solver:
    ...
    def loss(self, mu, coef_eq, tau, v):
        ...
        log_ll_eq = coef_eq * v * jnp.sum(jnp.square(-u_ddx1.reshape(-1) - u_ddx2.reshape(-1) + u.reshape(-1)**3 - self.f.reshape(-1)))
    ...
    def GN_loss(self, mu, mu_old, coef_eq, tau, v):
        ...
        log_ll_eq = coef_eq * v * jnp.sum(
            jnp.square(-u_ddx1.reshape(-1) - u_ddx2.reshape(-1) + u_old.reshape(-1) ** 3 + 3*(u_old.reshape(-1)**2)*p.reshape(-1) - self.f.reshape(-1)))
    ...
    def train(self, tau=5e6, v=8.0, mu_init="rdm", mu_var=0.1, ls_init=0.5, method="GD"):
        ...
        MSE_list = []
        if method == "GD":
            optimizer = optax.adamw(learning_rate=optax.exponential_decay(init_value=1e-3, transition_steps=3000,
                                                                          end_value=1e-5, decay_rate=0.9))
...
def run(num, Jitter, Ls):
    ...
    Y_test = vmap(u)(X_test[:, 0], X_test[:, 1]).reshape(200, 200)
    sensors_test = [X, T]

    solver = GP_Solver(X_test, Y_test, sensors_test, nu_idx=3, jitter=Jitter, num=num)
    min_err = solver.train(v=1e4, tau=1e12, mu_var=0.1, ls_init=Ls, mu_init="zeros", method="GN")


if __name__ == "__main__":
    num, Jitter, Ls = configs[0]
    random.seed(0)
    np.random.seed(0)
