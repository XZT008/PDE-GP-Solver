...
class Kernel(object):
    ...
    def get_derivative_cov(self, X, ls):
        num_s = X[0].shape[0]
        num_t = X[1].shape[0]
        x_p1 = jnp.tile(X[0].flatten(), (num_s, 1)).T
        X1_p1 = x_p1.flatten()
        X2_p1 = jnp.transpose(x_p1).flatten()

        x_p2 = jnp.tile(X[1].flatten(), (num_t, 1)).T
        X1_p2 = x_p2.flatten()
        X2_p2 = jnp.transpose(x_p2).flatten()

        K_u1 = vmap(self.K_u.kappa, (0, 0, None))(X1_p1.flatten(),
                                                  X2_p1.flatten(),
                                                  ls[0]).reshape(num_s, num_s)
        K_u2 = vmap(self.K_u.kappa, (0, 0, None))(X1_p2.flatten(),
                                                  X2_p2.flatten(),
                                                  ls[1]).reshape(num_t, num_t)
        K_dx1 = vmap(self.K_u.D_x1_kappa,
                     (0, 0, None))(X1_p1.flatten(), X2_p1.flatten(),
                                   ls[0]).reshape(num_s, num_s)
        K_dx2 = vmap(self.K_u.D_x1_kappa,
                     (0, 0, None))(X1_p2.flatten(), X2_p2.flatten(),
                                   ls[1]).reshape(num_t, num_t)
        K_ddx1 = vmap(self.K_u.DD_x1_kappa,
                      (0, 0, None))(X1_p1.flatten(), X2_p1.flatten(),
                                    ls[0]).reshape(num_s, num_s)
        K_dddx1 = vmap(self.K_u.DDD_x1_kappa,
                       (0, 0, None))(X1_p1.flatten(), X2_p1.flatten(),
                                     ls[0]).reshape(num_s, num_s)
        """
        K_ddddx1 = vmap(self.K_u.DDDD_x1_kappa,
                        (0, 0, None))(X1_p1.flatten(), X2_p1.flatten(),
                                      ls[0]).reshape(num_s, num_t)
        """
        u = [K_u1, K_u2]
        u_dx1 = [K_dx1, K_u2]
        u_dx2 = [K_u1, K_dx2]
        ...
        return [u, u_dx1, u_dx2, u_ddx1, u_dddx1]
